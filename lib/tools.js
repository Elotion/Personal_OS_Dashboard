...
const TOOL_DEFINITIONS = [
  ...
  {
    name: 'log_missed_sleep',
    description: 'Backdate a night of sleep that was never logged in real time at all -- Elo never clicked "went to bed" or "woke up" for it, so start_sleep/end_sleep ' +
      '(which only ever mean "right now") do not apply. Use this when Elo says he forgot to log a past night and tells you roughly what time he went to bed and woke up, ' +
      'or when answering a morning nudge asking exactly that. Times are approximate by nature -- do not ask him to be more precise than he already was.',
    input_schema: {
      type: 'object',
      properties: {
        logged_date: { type: 'string', description: 'YYYY-MM-DD -- the night this is for (the day he went to bed, not the day he woke up), e.g. "last night" relative to today' },
        bed_time: { type: 'string', description: '24-hour "HH:MM", e.g. "23:00" for 11pm or "01:30" for 1:30am -- convert whatever rough time Elo gives you' },
        wake_time: { type: 'string', description: '24-hour "HH:MM", e.g. "07:00" for 7am -- convert whatever rough time Elo gives you' },
        quality: { type: 'integer', minimum: 1, maximum: 5, description: 'Optional 1-5 sleep quality rating, only if Elo mentioned one' },
      },
      required: ['logged_date', 'bed_time', 'wake_time'],
      additionalProperties: false,
    },
  },
  {
...
const WRITE_TOOLS = new Set([
  'toggle_habit', 'toggle_habit_subtask', 'log_food', 'create_task', 'update_task', 'start_sleep', 'cancel_sleep', 'end_sleep', 'log_missed_sleep', 'create_journal_entry',
]);
...
const EXECUTORS = {
  ...
  // effectiveDate, not localDateStr(new Date()) -- a midnight snack logged
  // via Telegram before Elo has gone to bed should count toward the day
  // that, from his own perspective, hasn't ended yet (lib/habitDay.js's
  // rule, same as create_journal_entry below and the habit toggles above).
  log_food: async (input) => {
    const effectiveToday = input.logged_date || (await apiGet('/api/today')).date;
    return apiSend('POST', '/api/nutrition', {
  ...
  log_missed_sleep: async (input) =>
    apiSend('POST', '/api/sleep/log-missed-night', {
      logged_date: input.logged_date, bed_time: input.bed_time, wake_time: input.wake_time, quality: input.quality,
    }),
...
const SUMMARIZE = {
  ...
  log_missed_sleep: (input) => '🌙 Log missed night (' + input.logged_date + '): bed ~' + input.bed_time + ', woke ~' + input.wake_time,
