...
function journalDisplayFields(entryDateStr) {
  ...
  let day;
  if (entryDateStr === todayStr) day = 'TODAY';
...
const TOOL_DEFINITIONS = [
  ...
  {
    name: 'toggle_habit',
    description: 'Mark a specific habit done or not-done for today. Look up the real habit_id via get_habits first.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'start_sleep',
    description: 'Record that Elo is going to bed right now.',
    input_schema: { type: 'object', properties: {}, additionalProperties: false },
  },
  {
    name: 'end_sleep',
...
const WRITE_TOOLS = new Set([
  'toggle_habit', 'log_food', 'create_task', 'update_task', 'start_sleep', 'end_sleep', 'create_journal_entry',
]);
...
const EXECUTORS = {
  ...
  create_journal_entry: async (input) => {
    const entry_date = input.entry_date || localDateStr(new Date());
    const { day, date } = journalDisplayFields(entry_date);
