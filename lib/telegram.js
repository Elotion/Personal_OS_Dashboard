...
// Morning stale-task nudge (2026-08-31, Elo's request) -- deliberately NOT
// a generic "here's your day" briefing (he already checks CRM/calendar
// himself right after his routine, so that would be redundant); the one
// thing worth surfacing proactively is a task that's been sitting in
// "THIS WEEK" longer than he'd want, since that's easy to miss scanning
// the board casually. Timed off the actual wake-up event (a fixed delay
// after end_sleep executes), not a fixed clock time, matching how his
// routine length varies day to day and his explicit ask: he wants this
// to land "when my routine finished," not at some arbitrary hour that
// might catch him mid-yoga. Silent if nothing's actually stale, to
// respect the phone-free morning he described.
...
function scheduleMorningNudge(bot, chatId) {
  setTimeout(async () => {
    try {
      ...
      const stale = tasks.filter((t) => t.timeframe === 'THIS WEEK' && t.created_at && new Date(t.created_at).getTime() < cutoff);
      if (stale.length === 0) return;
      const lines = ['☀️ These have been sitting in THIS WEEK for over 7 days:', ...stale.map((t) => '• ' + t.title)];
