...
function start() {
  ...
  startSchedules(
    ...
    // For a nudge whose reply needs to be resolvable (currently just
    // missedSleepCheck) -- sends it AND records it into conversationHistory
    // as a real assistant turn (not a "(Note: ...)" bookkeeping wrapper --
    // this is genuinely what the bot said, verbatim, unlike those), so the
    // next runAgentTurn call for this chat can see what was actually asked.
    (chatId, text) => {
