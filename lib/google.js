...
// Today's events only, for HOME's calendar card -- widening to a date range
// is a one-line change here (start/end below) when that's actually needed.
...
// Google Tasks is a genuinely separate API/product from Calendar (different
// auth scope, different resource model) -- confirmed directly that items
// that LOOK like calendar events in Google Calendar's own UI (recurring
// class schedule blocks) don't exist as Calendar events on any calendar in
// this account, strongly suggesting they're actually Tasks with due dates,
// which Calendar's UI blends into the month view.
//
// The standard Tasks API only stores a DUE DATE, not a due time (`due` comes
// back as an RFC3339 timestamp but it's always midnight UTC representing
// "just a date" -- there's no way to recover a real clock time from it).
// Reading the date via `.slice(0, 10)` rather than `new Date(t.due)` is
// deliberate: converting through a Date object and reformatting would
// reintroduce the exact UTC-midnight-parses-as-a-different-local-day bug
// already fixed elsewhere in this app, since `t.due` is always UTC midnight.
async function listTasks() {
  ...
  for (const list of (listsRes.data.items || [])) {
    const res = await tasksApi.tasks.list({ tasklist: list.id, showCompleted: false, showHidden: false });
    (res.data.items || []).forEach((t) => {
      tasks.push({
        title: t.title || '(untitled)',
        dueDate: t.due ? t.due.slice(0, 10) : null,
        listName: list.title,
      });
    });
  }
...
module.exports = {
  getAuthUrl, saveTokens, isConnected, getAuthorizedClient, listTodayEvents, newOAuthClient,
  listCalendars, setHiddenCalendarIds, listTasks,
};
