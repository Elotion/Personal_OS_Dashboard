...
// Write-through history (2026-08-31, Elo's request) -- calendar was
// deliberately never persisted (HOME only ever needed a live "today" view),
// but there was nothing for the insight/correlation features to reason
// about ("does a busy calendar day affect habit completion?" was
// unanswerable). Rather than a separate background sync job (no scheduler
// existed in this app before now, and a real cron sync is more
// infrastructure than this needs), events get captured as a side effect of
// the SAME fetch HOME/Telegram already make -- upserted by google_event_id,
// so re-fetching the same day just refreshes the row instead of
// duplicating it. This means coverage depends on a day actually being
// viewed/asked about at least once -- a day nobody ever looks at has no
// history -- an accepted tradeoff, not a full sync.
async function persistEvents(dateStr, rows) {
...
async function listEventsForDate(dateStr) {
  ...
  for (const cal of visibleCalendars) {
    ...
    (res.data.items || [])
      ...
      .forEach((ev) => {
        ...
        // Google's dateTime strings carry their own offset (an absolute
        // instant) -- converted to local wall-clock text via
        // localTimestampStr(), matching how every other TIMESTAMP column in
        // this app stores time (see lib/dates.js), not a raw UTC ISO string.
        toPersist.push({
  ...
  persistEvents(date, toPersist).catch((e) => console.error('[calendar] persistEvents failed:', e));
