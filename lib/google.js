...
//
// Pulls from every calendar checked in the user's own Google Calendar sidebar
// (calendarList's `selected` flag -- default false, so this only includes ones
// explicitly turned on), not just the primary calendar. A single-calendar user
// wouldn't notice the difference, but this app's actual user keeps their real
// schedule on secondary calendars (School, Work, ...) with the primary one
// unchecked/empty -- querying only 'primary' silently returned nothing.
async function listTodayEvents() {
  ...
  const selectedCalendars = (calListRes.data.items || []).filter((c) => c.selected === true);
...
module.exports = { getAuthUrl, saveTokens, isConnected, getAuthorizedClient, listTodayEvents, newOAuthClient };
