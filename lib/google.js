...
async function listEventsForDate(dateStr) {
    ...
    (res.data.items || [])
      ...
      .forEach((ev) => {
        ...
        events.push({
          ...
          // The real Google calendar name (Work/School/Events/...) -- was
          // already known here and persisted via toStorableRow below, but
          // never actually reached the frontend on this live-fetch path
          // (the final .map() at the bottom of this function stripped it).
          // Elo, 2026-09-11: wants to know which calendar an event belongs
          // to at a glance, without reading the description.
  ...
  return events.map(({ time, label, calendarName }) => ({ time, label, calendarName }));
}
