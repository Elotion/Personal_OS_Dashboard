...
async function getHealthContext(days) {
  ...
  const nutritionResult = await supabase.from('nutrition_log')
    .select('kcal, protein, carbs, fat, logged_date').gte('logged_date', windowStartStr);
  if (!nutritionResult.error) {
    (nutritionResult.data || []).forEach((n) => {
      const d = nutritionByDay[n.logged_date] || { kcal: 0, protein: 0, carbs: 0, fat: 0 };
      d.kcal += n.kcal || 0; d.protein += n.protein || 0; d.carbs += n.carbs || 0; d.fat += n.fat || 0;
      nutritionByDay[n.logged_date] = d;
    });
  } else {
    throw nutritionResult.error; // nutrition_log has always existed, a real error here is real
  }
