...
// Catches the exact failure class documented below: the model echoing an
// injected (Note: ...) history line back as if it were a fresh reply,
// instead of taking a real action. A code-level net, not just a prompt
// instruction -- instructions reduce this but don't eliminate it, and the
// user should never see raw internal bookkeeping text either way.
const NOTE_LEAK_PATTERN = /proposed but not yet confirmed|\(Note:/i;
...
async function systemPrompt() {
  ...
  return (
    ...
    'A completed_at TIMESTAMP can show a literal calendar date one day later than completed_date -- e.g. ' +
    'completed_at 2026-09-06 01:51 on a habit/sub-task whose completed_date is still 2026-09-05, because ' +
    "he did it just after midnight, before going to bed. Never call that \"yesterday's session\" or " +
    'similar -- to him it is the SAME session, right before bed, not a separate earlier day. Describe it ' +
    'as already done / done earlier tonight instead. ' +
    ...
    'start_sleep/end_sleep only ever mean "right now" -- if Elo is instead telling you about a PAST night he ' +
    'never logged at all (he forgot, or he is answering a nudge asking about it), use log_missed_sleep instead, ' +
    'never start_sleep/end_sleep, converting whatever rough times he gives into 24-hour HH:MM. ' +
...
async function runAgentTurn(userText, history = []) {
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    if (toolUses.length === 0) {
      ...
      // Real bug caught live (2026-09-06), a recurrence of the same class
      // documented below for a different action (end_sleep, not start_sleep
      // this time): despite the system-prompt instruction, the model can
      // still echo an injected (Note: ...) line back as its own reply,
      // instead of actually calling the tool -- e.g. "woke up" got the
      // literal text "(Note: proposed but not yet confirmed -- end_sleep({}))"
      // sent to Elo, with no real tool call and no button to confirm anything.
      // One retry with an explicit correction nudge, since a fresh attempt
      // frequently self-corrects; if it STILL leaks, never forward the raw
      // internal text to Elo -- fall back to a safe, honest message instead.
      if (NOTE_LEAK_PATTERN.test(text)) {
        console.error('[agent] note-leak caught, retrying once:', text);
        const corrected = await client.messages.create({
          ...
          messages: [
            ...
            { role: 'user', content: 'That reply just repeated internal bookkeeping text instead of a real answer -- (Note: ...) lines are historical context only, never something to say back to me. Actually call the right tool for my last message, or ask me a real clarifying question in your own words.' },
          ],
        });
        // If the retry produced a real write tool call, that's the actual
        // recovery -- propose it exactly like the normal write-call path
        // below (same (Note: ...) history bookkeeping, same confirm-card
        // return shape), just reached via this retry instead of the main
        // loop. A retry that comes back with only read calls or more leaked
        // text is rare enough that it isn't worth fully re-entering the
        // loop for -- falls through to a safe generic message instead.
        const retryWriteCalls = corrected.content.filter((b) => b.type === 'tool_use' && WRITE_TOOLS.has(b.name));
        if (retryWriteCalls.length > 0) {
          const summary = '(Note: proposed but not yet confirmed by button tap -- ' +
            retryWriteCalls.map((b) => b.name + '(' + JSON.stringify(b.input) + ')').join('; ') + ')';
          messages.push({ role: 'assistant', content: summary });
          return { type: 'actions', actions: retryWriteCalls.map((b) => ({ id: b.id, name: b.name, input: b.input })), history: messages };
        }
        const retryText = corrected.content.filter((b) => b.type === 'text').map((b) => b.text).join('\n').trim();
        text = NOTE_LEAK_PATTERN.test(retryText) || !retryText
          ? "Sorry, I got confused there -- could you tell me again what you'd like me to do?"
          : retryText;
      }
    ...
    if (writeCalls.length > 0) {
      ...
      // read, never a line to continue or imitate. This still recurred once
      // for a different action (end_sleep, 2026-09-06) despite the wrapper
      // and instruction, so the code-level NOTE_LEAK_PATTERN retry/fallback
      // above this function is the real backstop -- this phrasing tweak
      // (matching the retry path's own wording) is a second, complementary
