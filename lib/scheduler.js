...
const { syncCalendarWindow } = require('./google');
...
// Missed-sleep morning check, 8am daily (2026-09-08, Elo -- he forgot to log
// a real night entirely, and the dashboard stayed frozen on the prior day
// until he manually clicked bed-then-immediately-wake just to force a
// refresh). Only ever asks about the single most recent possible night
// (yesterday, by real calendar date) -- if ignored, tomorrow's run asks
// about the new "yesterday" instead of re-asking about the same one
// forever, so this can't spiral into repeat nagging over one missed night.
// Silent if a night is genuinely still in progress (a real pending bedtime
// right now), or if there's no sleep history at all yet (nothing to
// consider "missed" on day one of ever using this feature).
// Uses `record`, not the plain `sendMessage` every other nudge here uses --
// unlike those, a reply to THIS one ("around 11 and 7") is meaningless
// without knowing which night it's answering, so the sent text itself needs
// to land in conversationHistory for the next runAgentTurn call to see, not
// just reach Telegram.
async function missedSleepCheck(record) {
  const [pending, log] = await Promise.all([apiGet('/api/sleep/pending'), apiGet('/api/sleep')]);
  if (pending.bed_time) return; // a night is genuinely in progress right now
  if (log.length === 0) return; // never used sleep tracking yet -- nothing to call "missed"

  const now = new Date();
  const lastNight = localDateStr(new Date(now.getFullYear(), now.getMonth(), now.getDate() - 1));
  if (log.some((s) => s.logged_date === lastNight)) return; // already logged

  await record(
    '🌙 Looks like the night of ' + lastNight + " never got logged — what time did you go to bed that night, " +
    'and what time did you wake up the next morning? Roughly is fine.'
  );
}

function startSchedules(getClaimedChatId, sendMessageTo, recordMessageTo) {
  ...
  // Same shape as withChat, but for a nudge whose reply needs to be
  // resolvable -- see missedSleepCheck's own comment for why.
  async function withChatRecording(fn) {
    const chatId = await getClaimedChatId();
    if (!chatId) return;
    await fn((text) => recordMessageTo(chatId, text)).catch((e) => console.error('[scheduler] failed:', e));
  }
  ...
  cron.schedule('0 8 * * *', () => withChatRecording(missedSleepCheck), { timezone: TIMEZONE });

  // Real calendar history sync, 3am daily (2026-08-31, Elo: "why can't we
  // have the memory of the entire google calendar" -- a fair question, so
  // this replaces relying on someone happening to view a given day).
  // No chat/message involved -- pure background maintenance, logged
  // server-side only, same as any other scheduled job with nothing for
  // Elo to act on.
  cron.schedule('0 3 * * *', async () => {
    try {
      const result = await syncCalendarWindow();
      console.log('[scheduler] calendar sync:', JSON.stringify(result));
    } catch (e) {
      console.error('[scheduler] calendar sync failed:', e);
    }
  }, { timezone: TIMEZONE });

  console.log('[scheduler] evening check-in (9pm), food reminders (10am/2pm/8pm), missed-sleep check (8am), calendar sync (3am) scheduled');
}
